...
public class RegrasJogo {
    ...
    private void cadastrarJogadores(double saldoInicial) {
        ...
        System.out.print("Num de jogadores: ");
        int numeroJogadores = scanner.nextInt();

        for (int i = 0; i < numeroJogadores; i++) {
            System.out.print("Nome do player " + (i + 1) + ": ");
        ...
        while (rodadaAtual < rodadasMax && jogadores.size() > 1) {
            ...
            for (Jogador jogador : jogadores) {
                if (jogador.estaFalido()) {
                    System.out.println(jogador.nome + " está falido e foi excluido.");
                    continue;
                }


                if (jogador.estaNaPrisao()) {
                    System.out.println(jogador.nome + " está preso e não pode participar desta rodada.");
            ...
            case "IMOVEL":
                if (casa.getImovel().getProprietario() == null) {
                    if (jogador.getSaldo() >= casa.getImovel().getPrecoCompra()) {
                        ...
                        System.out.println(jogador.getNome() + " adquiriu " + casa.getNome());
                    } else {
    ...
    private void declararVencedor() {
        ...
        if (vencedor == null) {
            System.out.println("Exatamente todos jogadores foram à falencia. Não há um vencedor.");
        } else {
